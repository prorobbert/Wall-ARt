...
class PersistenceManager {
    ...
    // The JSON file to store the world anchor to plaed object mapping.
    ...
    @MainActor
    func attachPersistentObjectToAnchor(_ modelFileName: String, anchor: WorldAnchor) {
    ...
    func process(_ anchorUpdate: AnchorUpdate<WorldAnchor>) {
        ...
        case .added:
            ...
            // ARKit surfaces all of the world anchors associated with this app when the world tracking provider starts.
            if let persistedObjectFileName = persistedObjectFileNamePerAnchor[anchor.id] {
                attachPersistentObjectToAnchor(persistedObjectFileName, anchor: anchor)
            ...
            fallthrough
        case .updated:
            // Keep the position of placed objects in sync with their corresponding world anchor, and hide the object if the anchor isn't tracked.
    ...
    func removeAllPlacedObjects() async {
        ...
        // The placed objects will then be removed after the world anchors were successfully deleted.
    ...
    func attachObjectToWorldAnchor(_ object: PlacedObject) async {
        ...
        do {
            ...
            // Adding world anchors can fail, such as when you reach the limit for total world anchors per app.
            // Keep track of all world anchors and delete any that no longer have an object attached.
...
"""
    ...
    private func detachObjectFromWorldAnchor(_ object: PlacedObject) {
        ...
        Task {
            // The world anchor is no longer needed; remove it so that it doesn't remain in the app's list of world anchors forever.
            await removeAnchorWithID(anchorID)
        }
    }
    ...
    func checkIfAnchoredObjectsNeedToBeDetached() async {
        ...
        // Check if any of the anchored objects is no longer at rest and needs to be detached from its world anchor.
    ...
    func checkIfMovingObjectsCanBeAnchored() async {
        ...
        // Check whether any of the nonanchored objects are now at rest and can be attached to a new world anchor.
